...
public class ThirdPersonController : MonoBehaviour
{
    ...
    private float GetCurrentSpeed()
    {
        ...
        if (animator.GetBool(AnimationType.Crunch.ToString()))
        {
            ...
            if (input.IsSprint)
            {
                targetSpeed = MoveSpeed * 2; // ถ้า sprint ระหว่าง crunch ให้ speed เท่ากับ crunch speed คูณ 2
            }
        }
        else if (animator.GetBool(AnimationType.Prone.ToString()))
        {
            targetSpeed = MoveSpeed / 2; // ถ้า prone ให้ speed เท่ากับ speed ครึ่งหนึ่ง
        }
        else
        {
            targetSpeed = input.IsSprint ? SprintSpeed : MoveSpeed; // ถ้า sprint ให้ speed เท่ากับ SprintSpeed มิฉะนั้นให้ speed เท่ากับ MoveSpeed
        }
    ...
    public void Crouch(bool isCrouching)
    {
        ...
        if (isCrouching)
        {
            // ปรับขนาด CharacterController ลง
            controller.center = new Vector3(0, 0.5f, 0);
            controller.height = 1.0f;
            // ปรับ Offset ของ CharacterController ลง
        }
        else
        {
            // คืนค่าขนาด CharacterController เป็นค่าปกติ
            controller.center = new Vector3(0, 1, 0);
            controller.height = 2.0f;
            // คืนค่า Offset ของ CharacterController เป็นค่าปกติ
        }
    ...
    {
        // ปรับส่วนสูงของ CharacterController หรือยืดตัวลงตามที่ต้องการ
        Debug.Log("Prone == " + isProne);
        if (isProne)
        {
            // ปรับขนาด CharacterController ลง
    ...
    {
        Debug.Log("SetSprintAnimation == " + run);
